...
from u2net_data_loader import ToTensor
...
def main():
    ...
    prediction_dir = os.path.join(os.getcwd(), 'data', model_name + '_results' + os.sep)
    model_dir = os.path.join(os.getcwd(), model_name + '.pth')
    

    img_name_list = glob.glob(image_dir + os.sep + '*')
    for ind in range(len(img_name_list)):
        print(">>>", ind)

        # --------- 2. dataloader ---------
        test_salobj_dataset = SalObjDataset(img_name_list=img_name_list[ind:ind+1],
                                            lbl_name_list=[],
        ...
        for i_test, data_test in enumerate(test_salobj_dataloader):
            ...
            print("inferencing:", img_name_list[ind].split(os.sep)[-1])
            ...
            save_output(img_name_list[ind], pred, prediction_dir)
